...
const createMockChange = (beforeData, afterData) => ({
    before: {
        data: () => beforeData,
...
const createMockDocumentSnapshot = (data) => ({
    data: () => data,
    exists: data !== null,
    id: 'test-doc-id',
    ref: {
...
describe('Firestore Triggers', () => {
    ...
    describe('onTaskComplete Trigger', () => {
        ...
        it('should handle errors gracefully', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('❌ Error updating user stats on task completion:', expect.any(Error));
        ...
        it('should log completion event', async () => {
            ...
            const { onTaskComplete } = require('../../index');
            await onTaskComplete(mockChange, mockContext);
            expect(consoleSpy).toHaveBeenCalledWith('📝 Task task-log-test completed for user user-log-test');
            expect(consoleSpy).toHaveBeenCalledWith('✅ Updated completion stats for user user-log-test');
            consoleSpy.mockRestore();
        });
    ...
    describe('onUserCreate Trigger', () => {
        it('should create default tags for new user', async () => {
            ...
            firebase_mock_1.default.firestore().collection().doc.mockReturnValue(mockTagRef);
            ...
            expectedTags.forEach((tag, index) => {
                expect(mockBatch.set).toHaveBeenNthCalledWith(index + 1, mockTagRef, expect.objectContaining({
                    id: 'tag-id-123',
                    userId: 'new-user-123',
                    name: tag.name,
                    color: tag.color,
        ...
        it('should handle tag creation errors gracefully', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('❌ Error setting up new user error-user-123:', expect.any(Error));
            consoleSpy.mockRestore();
        });
        it('should create unique tag IDs for each tag', async () => {
            ...
            firebase_mock_1.default.firestore().collection().doc.mockImplementation(() => {
                return mockTagRefs[callCount++];
            });
            const { onUserCreate } = require('../../index');
            await onUserCreate(mockSnapshot, mockContext);
            // Verify each tag was created with unique ID
            expect(mockBatch.set).toHaveBeenNthCalledWith(1, mockTagRefs[0], expect.objectContaining({ id: 'tag-work-id', name: 'work' }));
            expect(mockBatch.set).toHaveBeenNthCalledWith(2, mockTagRefs[1], expect.objectContaining({ id: 'tag-personal-id', name: 'personal' }));
            expect(mockBatch.set).toHaveBeenNthCalledWith(3, mockTagRefs[2], expect.objectContaining({ id: 'tag-urgent-id', name: 'urgent' }));
        });
        it('should set correct timestamps for all tags', async () => {
            ...
            firebase_mock_1.default.firestore.Timestamp.now.mockReturnValue(mockTimestamp);
            const newUserData = {
                uid: 'timestamp-user',
            ...
            for (let i = 1; i <= 3; i++) {
                expect(mockBatch.set).toHaveBeenNthCalledWith(i, expect.any(Object), expect.objectContaining({
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle concurrent task completions', async () => {
            ...
            const promises = tasks.map(({ beforeData, afterData, context }) => onTaskComplete(createMockChange(beforeData, afterData), context));
...
//# sourceMappingURL=firestoreTriggers.test.js.map