...
describe('TaskHandlers', () => {
    ...
    beforeEach(() => {
        ...
        mockTaskService.createTask.mockResolvedValue({
            success: true,
            data: testHelpers_1.TaskTestDataGenerator.generateTask({ id: 'new-task-id', userId: testUserId })
        });
        mockTaskService.getUserTasks.mockResolvedValue({
            success: true,
            data: testHelpers_1.TaskTestDataGenerator.generateMultipleTasks(3, { userId: testUserId })
        });
        mockTaskService.updateTask.mockResolvedValue({
            success: true,
            data: testHelpers_1.TaskTestDataGenerator.generateTask({ id: 'updated-task-id', userId: testUserId })
        });
    ...
    describe('POST /tasks', () => {
        ...
        it('should schedule notifications for tasks with due dates', async () => {
            const taskWithDueDate = testHelpers_1.TaskTestDataGenerator.generateTask({
        ...
        it('should not schedule notifications for tasks without due dates', async () => {
            const taskWithoutDueDate = testHelpers_1.TaskTestDataGenerator.generateTask({ dueDate: null });
            mockTaskService.createTask.mockResolvedValue({
                success: true,
    ...
    describe('PUT /tasks/:taskId', () => {
        ...
        it('should update notifications when deadline changes', async () => {
            const updatedTask = testHelpers_1.TaskTestDataGenerator.generateTask({
                id: taskId,
                dueDate: new Date('2024-12-31T10:00:00Z')
            });
            mockTaskService.updateTask.mockResolvedValue({
    ...
    describe('DELETE /tasks/:taskId', () => {
        ...
        it('should delete a task successfully', async () => {
            const response = await (0, supertest_1.default)(app).delete(`/tasks/${taskId}`);
            expect(response.status).toBe(200);
            expect(response.body.success).toBe(true);
    ...
    describe('Tag Routes', () => {
        describe('GET /tasks/tags', () => {
            it('should get user tags successfully', async () => {
                const response = await (0, supertest_1.default)(app).get('/tasks/tags');
                expect(response.status).toBe(200);
                expect(response.body.success).toBe(true);
                expect(response.body.data).toBeDefined();
        ...
        describe('POST /tasks/tags', () => {
            it('should create a tag successfully', async () => {
                const tagData = testHelpers_1.TagTestDataGenerator.generateCreateTagData({
                    name: 'new-tag',
                    color: '#FF0000'
                });
                mockTagService.createTag.mockResolvedValue({
    ...
    describe('Notification Routes', () => {
        describe('POST /tasks/notifications/register-token', () => {
            it('should register device token successfully', async () => {
                ...
                expect(response.status).toBe(200);
                expect(response.body.success).toBe(true);
                expect(mockNotificationService.registerDeviceToken).toHaveBeenCalledWith(testUserId, 'device-token-123', 'ios');
            });
...
//# sourceMappingURL=taskHandlers.test.js.map